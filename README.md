...
OBS録画開始・停止を一画面で管理できるWindows向けウィジェットです。

ゲームを選んで起動するだけで「いつ、どのゲームを、どれくらい遊んだか」を残せるように作っています。
...
`shorts_agent/` には、録画済みゲーム動画からショート動画を作る補助CLIがあります。

使う場合はFFmpeg/FFprobeが必要です。VOICEVOX音声は任意です。元動画を上書きしない設計ですが、出力先は必ず `outputs/` 以下にしてください。

詳しくは [shorts_agent/README.md](shorts_agent/README.md) を見てください。
