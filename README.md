...
- 登録したゲームをウィジェットから起動
- 登録したゲームプロセスの開始から終了までをプレイ時間として自動記録
- 最後に遊んだゲームを上に表示
- 攻略サイト、メモ、スプレッドシートなどのリンクをゲームごとに表示
- OBS Studioの起動、ゲームプロセス開始時の録画開始、録画停止を自動化
- ゲーム終了時にOBS録画を自動停止
...
2. `ゲーム追加` を選びます。
3. `ゲーム名` を入力します。
4. `ランチャーexe または ゲームexe の場所` の `参照` から起動に使う `.exe` を選びます。
5. ゲームランチャーがある場合はランチャーexeを優先し、ない場合はゲームexeを選びます。
6. `プロセス名` には、プレイ時間とOBS録画開始の基準にするゲーム本体のプロセス名を入力します。
7. 必要なら攻略サイトやメモのリンクを追加します。
8. `作成` を押します。
...
登録した `プロセス名` のゲームプロセスが起動した時に録画を開始し、そのプロセスが終了した時に録画を停止できます。
...
`configs/example.json` は公開用のサンプル設定です。実際のゲーム設定は `configs/*.json` の個人設定として扱ってください。
